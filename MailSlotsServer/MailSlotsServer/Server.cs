...
namespace MailSlots
{
    public partial class frmMain : Form
    {
        ...
        private List<Int32> MailSlots = new List<Int32>();
            ...
            while (_continue)
            {
                ...
                if (MessageCount > 0)
                    for (int i = 0; i < MessageCount; i++)
                    {
                        ...
                        if (system_msg.Length > 1 && system_msg[1].Contains("system_message"))
                        {

                            string userName = system_msg[0];
                            Users.Add(userName);
                            //MailSlots.Add(DIS.Import.CreateFile($"\\\\.\\mailslot\\{userName}", DIS.Types.EFileAccess.GenericWrite, DIS.Types.EFileShare.Read, 0, DIS.Types.ECreationDisposition.OpenExisting, 0, 0));
                            SendMessageToUser($"{userName} подключился к чату!");
        ...
        private void SendMessageToUser(string message)
        {
            foreach (var user in Users)
            {
                ...
                Int32 channel = DIS.Import.CreateFile($"\\\\.\\mailslot\\{user}", DIS.Types.EFileAccess.GenericWrite, DIS.Types.EFileShare.Read, 0, DIS.Types.ECreationDisposition.OpenExisting, 0, 0);
                DIS.Import.WriteFile(channel, buff, Convert.ToUInt32(buff.Length), ref BytesWritten, 0);
                DIS.Import.CloseHandle(channel);
            }
        }
        ...
        private void frmMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (ClientHandleMailSlot != -1)
            {
                ...
                foreach(var user in MailSlots)
                {
                    DIS.Import.CloseHandle(user);
                }
            }
        }
    }
